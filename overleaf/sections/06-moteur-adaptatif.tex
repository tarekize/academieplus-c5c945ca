...
Ce chapitre répond directement à la question \emph{\og comment est calculé le
niveau de l'élève~?\fg{}}. Toute la logique de calcul est concentrée dans des
...
Le \og score attendu\fg{} suit une logistique de type ELO~:
\[
...
\begin{itemize}
  \item \textbf{\og timeout\fg{}} (exercice ouvert $>1$\,min sans réponse puis quitté)~:
        $-1$, ou $-2$ si difficulté $\geq 4$.
  \item \textbf{\og erase\fg{}} (saisie puis effacement total puis sortie)~: $0$ sur le
...
volontairement sur le bouton \og Voir la solution détaillée\fg{} (ce qui compte alors
comme un abandon). Dans les quiz, l'élève sélectionne une option puis confirme
via le bouton \og Confirmer\fg{}.
\end{notebox}
