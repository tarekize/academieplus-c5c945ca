-- BUG : redeem_activation_code() et admin_grant_subscription_days() créditaient
-- le temps écoulé depuis last_tick_at dans days_used sans jamais le plafonner
-- à total_days. Résultat : un abonnement resté expiré longtemps (jours_used
-- qui continue de croître avec le temps réel écoulé, même sans jours restants
-- à consommer) accumule une "dette" négative illimitée. Réactiver un nouveau
-- code n'ajoute alors que ses propres jours sur cette dette, sans jamais
-- repasser actif si la dette dépasse les jours du nouveau code — un élève qui
-- vient de payer/activer un code se retrouve encore affiché "expiré".
-- Corrigé en plafonnant le temps crédité à days_used à total_days (avant
-- d'ajouter les nouveaux jours) : le solde restant ne peut jamais descendre
-- sous 0, donc chaque activation redonne exactement ses jours, quelle que
-- soit la durée d'expiration précédente.
...
-- Répare la seule ligne actuellement affectée par ce bug (compte de test) :
-- son dernier code activé (70 jours cumulés) a été immédiatement rongé par
-- la dette d'expiration accumulée. On lui redonne les jours qu'il vient de
-- payer, à compter de maintenant.
UPDATE public.student_subscriptions
SET days_used = 0, last_tick_at = now()
WHERE is_paused = false AND days_used > total_days;
