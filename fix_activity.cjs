...
txt = txt.replace(
    ...
    function (match) {
        if (match.includes('showHint')) return match; // already done
        return match + `
          {question.hint && (
...
txt = txt.replace(
    ...
    '<div className="flex gap-1"><span className="font-medium whitespace-nowrap">الشرح: </span><HtmlWithMath htmlContent={explanation} className="flex-1" /></div>'
);

txt = txt.replace(
    /<div><span className="font-medium">الشرح: <\/span>\{question\.explanation\}<\/div>/g,
    '<div className="flex gap-1"><span className="font-medium whitespace-nowrap">الشرح: </span><HtmlWithMath htmlContent={question.explanation} className="flex-1" /></div>'
);
...
txt = txt.replace(
    ...
    `{exercise.statement.includes('<') || exercise.statement.includes('$') || exercise.statement.includes(' \\\\') ? (
            ...
            <p className="text-sm border-t pt-2" dir="rtl">{exercise.statement}</p>
          )}
          {exercise.hint && (
            <div dir="rtl" className="mt-2 mb-3">
              ...
              {showHint && (
                <div className="p-3 rounded-lg bg-amber-500/10 border border-amber-500/30">
                  <div className="flex items-start gap-2">
                    <Lightbulb className="h-4 w-4 text-amber-500 mt-1 shrink-0" />
                    <HtmlWithMath htmlContent={exercise.hint} className="text-sm" />
                  </div>
                </div>
              )}
            </div>
...
txt = txt.replace(
    ...
    `{solution && (
                <div className="flex flex-col gap-1 border-t pt-2 mt-2">
                  ...
                  <HtmlWithMath htmlContent={solution} className="bg-white dark:bg-black/20 p-3 rounded-md border border-purple-100 dark:border-purple-900/30" />
                </div>
              )}`
);
...
txt = txt.replace(
    ...
    `{exercise.solution && (
                <div className="flex flex-col gap-1 border-t pt-2 mt-2">
                  <span className="font-medium text-purple-700 dark:text-purple-400">الحل المفصل 🎯</span>
                  <HtmlWithMath htmlContent={exercise.solution} className="bg-white dark:bg-black/20 p-3 rounded-md border border-purple-100 dark:border-purple-900/30" />
                </div>
              )}`
);
