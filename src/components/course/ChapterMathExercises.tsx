      ...
      <Card className="max-w-3xl mx-auto">
        <CardHeader className="pb-3">
          <div className="flex items-center justify-between">
            <Button variant="ghost" size="sm" onClick={() => setCurrentExercise(null)}>
              <ArrowLeft className="h-4 w-4 mr-2" />اÙ„عÙˆدة Ù„Ù„Ù‚ائÙ…ة
            </Button>
            <span className="text-sm px-2 py-1 rounded-full bg-primary/10 text-primary">
              تÙ…رÙŠÙ† {currentExercise + 1}/{exercises.length}
            </span>
          </div>
        ...
        <CardContent className="space-y-6">
          <div className="p-4 bg-muted/50 rounded-lg" dir="rtl">
            <h4 className="font-semibold mb-3 flex items-center gap-2"><BookOpen className="h-4 w-4" />اÙ„تÙ…رÙŠÙ†</h4>
          ...
          <div className="space-y-4" dir="rtl">
            <div className="flex gap-3">
              <Input placeholder="أدخÙ„ إجابتÙƒ..." value={userAnswers[exercise.id] || ""}
            ...
            {submitted && (
              <div className={cn("p-4 rounded-lg flex items-center gap-3", correct ? "bg-green-500/10 border border-green-500/30" : "bg-red-500/10 border border-red-500/30")}>
                {correct ? (
                  <><CheckCircle2 className="h-5 w-5 text-green-500 shrink-0" /><div><p className="font-medium text-green-700 dark:text-green-300">إجابة صحÙŠحة! ðŸŽ‰</p>{expectedAnswer && <p className="text-sm text-muted-foreground">اÙ„إجابة اÙ„Ù…تÙˆÙ‚عة: {expectedAnswer}</p>}</div></>
                ) : (
                  <><XCircle className="h-5 w-5 text-red-500 shrink-0" /><div><p className="font-medium text-red-700 dark:text-red-300">إجابة خاطئة</p><p className="text-sm text-muted-foreground">إجابتÙƒ: {userAnswers[exercise.id]}{expectedAnswer && ` â€” اÙ„Ù…تÙˆÙ‚عة: ${expectedAnswer}`}</p></div></>
                )}
              </div>
            )}
          </div>
          ...
          <Button variant="outline" onClick={() => toggleCorrection(exercise.id)} disabled={!submitted} className={cn("w-full gap-2", !submitted && "opacity-50")}>
            <Eye className="h-4 w-4" />{correctionVisible ? "إخفاء اÙ„حÙ„" : "عرض اÙ„حÙ„"}
          </Button>

          {correctionVisible && solution && (
            <div className="p-4 bg-muted/30 rounded-lg border" dir="rtl">
              <h4 className="font-semibold mb-3 flex items-center gap-2 text-primary"><CheckCircle2 className="h-4 w-4" />اÙ„حÙ„ اÙ„Ù…فصÙ„</h4>
              <div className="prose prose-sm dark:prose-invert max-w-none"><ReactMarkdown>{solution}</ReactMarkdown></div>
            </div>
          )}

          <div className="flex gap-3">
            {currentExercise > 0 && <Button variant="outline" onClick={() => setCurrentExercise(currentExercise - 1)} className="flex-1">اÙ„تÙ…رÙŠÙ† اÙ„سابÙ‚</Button>}
            {currentExercise < exercises.length - 1 && <Button onClick={() => setCurrentExercise(currentExercise + 1)} className="flex-1">اÙ„تÙ…رÙŠÙ† اÙ„تاÙ„ÙŠ</Button>}
          </div>
        </CardContent>
      </Card>
    );
      ...
      <CardHeader>
        <div className="flex items-center justify-between">
          ...
          <div className="flex gap-2">
            ...
            <Button variant="outline" onClick={onClose}>اÙ„عÙˆدة Ù„Ù„درس</Button>
          </div>
        </div>
        <p className="text-muted-foreground mt-2" dir="rtl">{chapterTitle} â€” {completedCount}/{exercises.length} Ù†اجح</p>
      </CardHeader>
      <CardContent>
        {exercises.length === 0 ? (
          <p className="text-center text-muted-foreground py-8" dir="rtl">Ù„ا تÙˆجد تمارين Ù…تاحة Ù„Ù‡ذا اÙ„فصÙ„.</p>
        ) : (
