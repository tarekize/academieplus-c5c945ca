...
import {
  ArrowLeft, Loader2, Users, BookOpen, Trash2, Copy, School, Plus, HeartHandshake, Lock, QrCode,
} from "lucide-react";
...
export default function EstablishmentManager({ teacherId, onBack }: { teacherId: string; onBack: () => void; }) {
  ...
  if (detailStudent) {
    ...
    const sInitials = sName.split(" ").map((n) => n[0]).filter(Boolean).join("").toUpperCase().slice(0, 2);
    return (
      ...
      <div className="space-y-6">
        ...
        {selectedClass.join_code && (
          <Card className="overflow-hidden rounded-2xl border-border/60">
            <div className="bg-[image:var(--gradient-primary)] px-5 py-2.5">
              <p className="flex items-center gap-2 text-xs font-semibold uppercase tracking-wide text-primary-foreground/90">
                <QrCode className="h-3.5 w-3.5" /> Rejoindre la classe
              </p>
            </div>
              ...
              <div className="flex flex-wrap items-center gap-6">
                ...
                <div className="flex-1 min-w-[220px] space-y-2">
                  <div className="flex items-center gap-2">
                    <span className="font-mono text-2xl font-bold tracking-[0.2em]">{selectedClass.join_code}</span>
                    <Button variant="ghost" size="icon" className="h-8 w-8 rounded-lg"
                      onClick={() => { navigator.clipboard.writeText(selectedClass.join_code || ""); toast.success("Code copié"); }}>
                      <Copy className="h-4 w-4" />
                    </Button>
                  </div>
                  <p className="text-sm text-muted-foreground">Partagez ce code pour que vos élèves rejoignent la classe.</p>
                  <Button
                    variant="outline"
                    size="sm"
                  ...
                  >
                    <Copy className="h-3 w-3 mr-1" /> Copier le lien d'invitation
                  </Button>
                </div>
                {/* QR code */}
